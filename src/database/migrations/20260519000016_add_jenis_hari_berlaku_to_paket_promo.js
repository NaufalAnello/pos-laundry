exports.up = async function(knex) {
  const hasJenis = await knex.schema.hasColumn('paket_promo', 'jenis');
  if (!hasJenis) {
    await knex.schema.table('paket_promo', function(table) {
      table.string('jenis', 20).defaultTo('persen'); // persen | nominal | paket
    });
  }

  const hasHariBerlaku = await knex.schema.hasColumn('paket_promo', 'hari_berlaku');
  if (!hasHariBerlaku) {
    await knex.schema.table('paket_promo', function(table) {
